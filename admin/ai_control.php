...
<head>
    <meta charset="UTF-8">
    <title>AI Optimization Gate</title>
...
<body class="bg-gray-100 font-sans text-gray-900 flex flex-col min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-6 py-8 w-full space-y-6 flex-grow">
        <h1 class="sidebar-title font-bold text-lg text-[#bb1919]">Neural AI Steering Portal</h1>
        <div class="bg-white border p-6 rounded shadow-sm space-y-4">
            <p class="text-xs text-slate-500 font-light font-sans">Configure rules governing Gemini models steering parameters, context thresholds, and heuristic clickbait severity penalties.</p>
            <div class="space-y-3 font-mono text-xs text-slate-600">
                <div>
                    <strong>STRICTNESS WEIGHTS:</strong> BBC style journalism guidelines (No editorializing / objective report lines).
                </div>
                <div>
                    <strong>TARGET ENGINE:</strong> gemini-2.5-flash with background fallback simulations.
                </div>
            </div>
        </div>
    </main>
